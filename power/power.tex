...
One reason why there is such a variety of clustering algorithms is due in part to it being an undefined problem. A simple example of two equally valid but opposing definitions for a cluster would be to compare a clique vs.\ a biclique. A definition that attempts to keep edges within clusters, such as Equation~\ref{eq:modularity} perfectly captures cliques, but fails completely for bicliques, as none of the edges are within the clusters, instead all falling between them.
There are also
\begin{figure}
    % \centering
    % \includegraphics{}
    \caption{show a clique vs biclique to show difficulty in clustering. Also maybe show overlapping communities?}
...
\begin{figure}
    \centering
    \includegraphics[height=.95\textheight]{power/figures/ARI1.pdf}
...
\begin{figure}
    \centering
    \includegraphics[height=.95\textheight]{power/figures/ARI2.pdf}
...
\begin{figure}
    % \centering
    % \includegraphics{}
     \caption{TODO: show pruning example on lesmis, worse than Figure~\ref{fig:lesmis}}
     \label{fig:pruning}   
\end{figure}
...
This cannot be applied to a general dendrogram, and so we instead make use of another property of dissimilarity clustering: the fact that each parent cluster can store the dissimilarity between its two child clusters before the merge. This is usually thought of as the height of each branch in the dendogram, and can be used to recursively merge branches in the hierarchy if their height difference falls below a certain threshold. This follows a similar idea to Pons and Latapy \cite{Pons2006} who use the same branch height information to determine an optimal cutting point for forming flat clusters.

The next step is to lay out the hierarchy itself as a tree. Following Holten \cite{Holten2006}, a radial tree layout is chosen, and since each leaf of the tree represents a vertex of the original graph, they are first placed around the circumference of the circle. The order in which leaves are placed around the circle is very important, as it directly affects the proximity of edges to be bundles together. This is a similar problem to seriation in matrix plots \cite{Liiv2010}.
Obviously any cousin vertices should be placed next to each other, but this still leaves an exponential number of orderings because the order within groups of cousins is still undecided.
Fortunately, a neat solution exists to this problem, as there exists an algorithm that can find an optimal ordering. The definition of optimal in this case is the ordering that minimises the sum of the dissimilarity of adjacent leaves.
Remarkably, this can be computed in polynomial time using the algorithm of Bar-Joseph et al.\ \cite{Bar-Joseph2001}, which leverages the structure of the hierarchical dendrogram to formulate the task as a dynamic programming problem.
% This would not be applicable without a dendrogram, and so is another 

The next step is to position the branches of the tree. I tried many methods for determining branch placement, including the use of force-directed methods such as Tutte's algorithm as in Figure~\ref{fig:misc_force_layouts}, but the method settled upon was to use polar coordinates.
...
The resulting visualisations for the four benchmark graphs in Table~\ref{tab:bundle_graphs} can be seen in Figure~\ref{fig:untangled_hairballs}, and pseudocode for the entire process can be seen in Figure~\ref{fig:algorithm_heb}.
...
Ward is good, and also tends to give dendrograms that lend themselves more to the pruning in Section. The 
Pruning is not always high quality, and it can potentially merge every single branch into one if branch height differences are even. However this also corresponds to a poor quality clustering in general because it means that the algorithm could not find large groups of datapoints that were all close to each other.
...
Future work should include trying a larger benchmark, with more bipartite graphs (both real-world and generated).
Our benchmark also measures clustering, not bundling quality. Optimising a different loss function that directly relates to bundling, rather than a cluster definition would also be good.
...
mention that pruning is not the only way to do things e.g. skipping more control points or using a different type of spline. YMMV
...
The use of b-splines is also worth exploring, as will be done in Section~\ref{sec:bspline_details}.

END THIS CHAPTER WITH PSEUDOCODE
\url{github.com/jxz12/MyFirstThesis}
...
If the previous section was lossy compression, confluent drawing is lossless.
