...
An example of this technique can be seen in Figure~\ref{TODO}, which contains this visualisation applied to the source code of the Unity game engine. The hierarchical tree itself can be seen on the TODO hand side, to illustrate the layout and influence of the ARG on the final visualisation. TODO: the ordering around the circle is MEGA important, and having a hierarchy already gives you that. The colour along the lines is also mega important (if you expect to e.g. have clustered sources or targets).
