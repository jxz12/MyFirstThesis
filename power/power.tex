...
The hierarchical edge bundling of the previous section can be interpreted as \emph{lossy} compression, because the bundling of edges can obscure the exact endpoints of each edge. This is another reason why 
...
\begin{table}
    \centering
    \caption[Experimental results on greedy power graph decomposition]{hi}
    ...
    \begin{tabular}{ |c|c|c|c|c| } 
     ...
     \cline{2-5}
     & only $\kappa_{\cap}$ & $\kappa_{\cap}$ and $\kappa_{\triangle}$ & only $\kappa_{\cap}$ & $\kappa_{\cap}$ and $\kappa_{\triangle}$ \\ 
     \hline\hline
     \texttt{florentine} (20)
        % &\cellcolor{grey} 11 (4)
        % &\cellcolor{grey} 11 (4)
