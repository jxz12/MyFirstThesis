...
The best dissimilarity for Ward over all graphs is Euclidean on snapshot embedding with $t=5$ in Equation~\eqref{eq:walktrap}, and it seems the original authors in \citet{Pons2006} agree with this choice too, as they performed their analysis using either $t=2$ or $t=5$.
...
\subsection{Greedy agglomeration}
\label{sec:greedy_power}
...
The greedy process above will be the subject of contributions in Section~\ref{sec:greedy_heuristics}.
...
It will be shown in Section~\ref{sec:node_split} that the combination of B-splines and shortest paths introduces problems not fully explored by \citet{Bach2017}, causing the resulting drawings to violate the second and third conditions in the above definition. These problems will be solved, and the subclass of confluent drawings the algorithm can produce will be identified as a new classification known as \emph{power-confluent}. The corrected solution guarantees that the second condition is always satisfied, but still cannot guarantee the third, as discussed in Section~\ref{sec:power_planarity}.

\newpage
\section{Improvements}
This section will describe multiple improvements to the methods described in the previous section, which encapsulate the novel contributions from this chapter. Specifically, the speed and quality of the power decomposition algorithm will be improved in Section~\ref{sec:greedy_heuristics}, and two theoretical issues will be resolved for the conversion to a confluent drawing in Sections~\ref{sec:node_split} and~\ref{sec:short_circuits}.

It is important to note that these theoretical issues are rare in practical use cases of \citet{Bach2017}, and that the solutions proposed here will not often change the resulting drawings. When the issue does occur however, it changes the implied connectivity of the graph which is a serious error. Ensuring correctness is also necessary for the theoretical side of the method to further progress.

\subsection{Greedy heuristics}
\label{sec:greedy_heuristics}
The algorithm outlined in Section~\ref{sec:greedy_power} greedily selects the next merge based on the reduction in edges as in Equation~\eqref{eq:kappa_cap}. However this is not the only metric that the greedy algorithm can use to judge the quality of any given merge.
...
candidate merges\footnote{Due to the use of \texttt{std::unordered\_set} in C++, which may vary between implementations of the standard library \citep{Josuttis2012}.}, (Figure.~\ref{fig:pseudo_pgd}, line~\ref{code:pgd:top}) and so when multiple merges have the same score, the choice becomes nondeterministic.
% 
...
\begin{figure}
  ...
  % , albeit this specific case can easily be prevented by only preventing original nodes from being used as intermediate control points, as the original authors appear to have done in \citet[Fig.~2(c)]{Bach2017} for the edge $\{u,w\}$.
...
\begin{figure}
  ...
  % The same example graph as in \citet[Fig.~2]{Bach2017} and here Figure~\ref{fig:power_teaser} is used, shown in a radial layout to emphasize the tree structure of power groups.
...
It is also not clear how often the theoretical issues raised in Sections~\ref{sec:node_split} and~\ref{sec:short_circuits} would occur in real world data. The pathological power graph example in Figure~\ref{fig:overlap_sm} is clearly a rare situation, and it is further not known if the power graph construction algorithm outlined in Section~\ref{sec:greedy_power} and improved in~\ref{sec:greedy_heuristics} would ever produce such a power graph. Future work could include analysing a dataset of graphs and searching for whether/how often such a situation ever occurs.
