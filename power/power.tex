...
\begin{table}
    ...
    % \definecolor{grey}{RGB}{222,222,222}
    % \definecolor{grey}{RGB}{152,251,152} % mint :)
...
\begin{figure}
    ...
    The score function $\kappa$ is from Equation~\eqref{eq:kappa_full}, and the merge operation on line~\ref{pseudo:pgd:merge} is described in Section~\ref{sec:power_graph}, where any new module is parented to its merged children by adding them to its set of children $C$.
    In practice a redundant super-module is maintained, whose children are the top level modules $M_\mathrm{top}$ on line~\ref{pseudo:pgd:top}.}
...
Our final change involves bringing down the complexity of neighbour set intersection to $O(E)$ using hash sets, which reduces the complexity of the algorithm down slightly to $O(|V|^3|E|)$.
...
We resolve this ambiguity, and the following related problem, in Section~\ref{sec:hierarchical_routing}.
...
\begin{figure}
    ...
    Bottom: an example of a power graph that causes a similar ambiguity, where $c$ appears connected to $b$ because the edge $\{b,d\}$ short-circuits the nested structure of power groups (represented by blue boxes), causing $\{a,b\}$ to be routed incorrectly downwards, instead of as shown by the dashed line.
    % The correct direction that would not cause an issue is shown as a dashed line along the top.
    % Routing edges resulting from inclusion within a power group are in blue, and from connection to a power edge in orange.
    }
    \label{fig:overlap}
\end{figure}
...
\begin{figure}
    ...
    \caption[A full example of Figure~\ref{fig:overlap}]{A full example of Figure~\ref{fig:overlap}, bottom, without any redundant power groups.
...
This entire process is illustrated in Figure~\ref{fig:radial}, and pseudocode for the resulting algorithm can be seen in Figure~\ref{fig:pseudocode_pcd}, Algorithm~\ref{alg:pcd}.
...
\begin{figure}
    ...
    \caption[Pseudocode for power-confluent drawing]{Pseudocode for our conversion from a set of power graph modules (see the output of Algorithm~\ref{alg:pgd} in Figure~\ref{fig:pseudocode_pgd}) to a rendered drawing, described in Section~\ref{sec:hierarchical_routing}. 
    ...
    The split operation on line~\ref{pseudo:split} then moves $N^+$ and $N^{-*}$ to separate routing nodes, as in Figure~\ref{fig:radial}.
    In practice a shorter edge length is placed between split routing nodes, and so for line~\ref{pseudo:sgd} require a layout method that can embed edge lengths, such as the stress layout in Section~\ref{sec:stress_background}.
...
The choice of spline has not yet been discussed, for either the hierarchical bundling of Section~\ref{sec:hierarchical_clustering} or here for confluent drawings. 
The choice of spline for both in the literature is the \emph{B-spline}, which is a generalisation of B\'ezier curves that offers the two nice properties of convex hull and local control, previously mentioned in Section~\ref{sec:node_split}.
...
We first prove that splines of degree $p$ must share at least $p$ control points to overlap. A B-spline is a parametric curve that interpolates a number of control points by summing up contributions from a basis function for each (Figure~\ref{fig:basis}). As a consequence, the spline is constructed as a series of polynomial
...
curves, known as segments, which each satisfy
...
This proof is illustrated in Figure~\ref{fig:basis}. As a result, splines used must be quadratic ($p=2$) for node splitting (Figure~\ref{fig:nodesplit}) to work as intended.
...
This second method is illustrated in Figure~\ref{fig:basis}.
...
The benefit of cubic splines is that they are $C^2$ continuous, i.e.\  have no sudden jumps in curvature, and having this extra smoothness is more aesthetically pleasing. Regardless, for all figures here except for Figure~\ref{fig:basis} we use quadratic splines, and with the first method of joining to end points.

Note that drawing a spline for every edge is not the only way to render the graph, and also introduces a great deal of redundancy due to the large amount of overlap between edges. If a purely confluent drawing is desired, then it is not necessary to redraw overlapping segments.
Allowing bundles to be relaxed \cite[Fig.~18]{Bach2017} is, however, a useful option that rendering each edge does provide.
