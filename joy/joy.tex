...
This definition has been previously used to assess the similarity of layouts given any linear transformation \cite{Ortmann2017}, and is used in the game in order to align subsequent layouts. If there are different vertices between topologies, then the union of vertices is instead compared.
...
One definition of `complex' that is closely related to stability is $\rho\sqrt{nC}$, where $\rho$ is the standard deviation of the off-diagonals of the community matrix in Equation~\ref{eq:jacobian_evaluated}, $n$ is the number of species, and $C$ is the connectance, defined as the proportion of realised to possible interactions and thus $0\leq C\leq1$. This was shown by May to negatively correlate with the probability of stability as defined in Equation~\eqref{eq:lambda_stability}, and acted as the springboard for the complexity vs.\ stability debate.
This criterion has been taken and updated to more accurately match real ecosystem structure \cite{Allesina2012, Tang2014Correlation}, but all contain the aforementioned components $\rho$, $n$, and $C$.
...
% it was also unstable in that it killed off 
...
These two traits, body mass and interference, take a wide range of values: mass goes from one gram to one thousand kilograms, therefore spanning six orders of magnitude; interference goes from 1$\times$10$^{\text{--}5}$ to 1, spanning 5 orders of magnitude. This range for mass was chosen to capture species of almost all sizes in the natural world. Smaller species do exist and are vital to Earth's ecosystem (such as bacteria and the smallest insects) but they are left out of scope for the game. The range of interference values is set to span almost the entire range of values that the off-diagonals of the interaction matrix $\mathbf{A}$ can possibly take, when parameterised with Equation~\ref{eq:interaction_strength} using the range of body mass. This was done because it is not known what range of values interference takes in nature, and so the assumption that it does not differ too greatly from the magnitude of inter-species interaction strengths.
...
At this point it is worth mentioning that the number of plants was also limited to a maximum of three in any given level, and usually fewer. This was necessary here because there is no explicit competition between plants, i.e.\ a $(-,-)$ in the interaction matrix, and so there is no penalty at all for adding more of them.
...
what is the outcome? analyse the most successful strategies for structural (average chain length/trophic level, number of edges, all the niche model stuff) and dynamical features (average body mass/interferences, paradox of enrichment, flux/abundance)
