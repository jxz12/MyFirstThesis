...
Chapter~\ref{chap:joy} presents an application of network visualisation algorithms to a research-oriented video game called EcoBuilder. The objectives aimed at from the design of the game are first presented, and the research outcomes from releasing it to the public are then reported.

The broader literature review for the topics studied is split up across these chapters, in Sections~\ref{sec:nodes_background}, \ref{sec:edges_background}, \ref{sec:power_background}, and \ref{sec:joy_background}, each titled Background. A general context for the fields studied in this thesis as a whole can largely be garnered from reading each of the three individual chapters together, if desired. 
