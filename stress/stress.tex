...
This is a non-linear equation like the one in Equation~\eqref{eq:eades} or in~\cite{Fruchterman1991, Frick1995}, and so cannot be solved exactly using linear methods as with Equations~\eqref{eq:tutte} or~\eqref{eq:spectral}. However an immediate benefit to this formulation is that there are no extra parameters to 

Stress as an energy function is known to produce high-quality layouts~\cite{Brandes2009} without requiring extra input parameters such as those in Equation~\eqref{eq:eades}. However, it is known to be difficult to optimise due to an abundance of local minima~\cite{DeLeeuw1988, Gansner2005}, and also does not scale well to larger graphs due to the quadratic number of terms in the summation~\cite{Brandes2009, Hu2005}.
